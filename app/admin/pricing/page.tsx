...
export default function AdminPricingPage() {
  ...
  async function handleSave(kitId: string) {
    ...
    if (res.ok) {
      ...
      setToolkits((prev) => prev.map((t) => t.id === kitId ? { ...t, price_monthly: price } : t));
  ...
  return (
    <div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-6 text-xs text-amber-800">
        ...
        To change what customers are actually charged, update the price in{" "}
        <strong>Stripe Dashboard → Products</strong>.
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {toolkits.map((kit) => (
                <tr key={kit.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <code className="text-xs text-gray-400 truncate max-w-[160px] block">
                      {kit.stripe_price_id ?? "—"}
                    </code>
                  </td>
                    ...
                    {editingId === kit.id ? (
                      <div className="flex gap-2 justify-end">
                        ...
                        <button
                          ...
                          className="text-xs border border-gray-200 text-gray-600 px-3 py-1 rounded-lg hover:border-gray-400"
                        >
                          Cancel
                        </button>
                      </div>
