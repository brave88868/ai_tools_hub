...
- SPEC-UI-APR08 ✅ 全站UI可读性提升（2026-04-08）：globals.css html font-size=18px（全站字体等比放大）+ 全站灰色正文文字深色化（gray-400→gray-700/gray-500→gray-700/gray-600→gray-800，覆盖components/home/*.tsx + Footer + tools/toolkits/blog/pricing/features/submit页面）+ FinalCTA改为全宽(w-full) + py-16内边距 + 文字改白色(text-white/opacity-70) + 信任文字更新为"3 free uses/day · No credit card · SSL secured"
- SPEC-SEO-USE-CASES ✅ use-case SEO页面批量生成（2026-04-08）：scripts/generate-use-cases-missing.mjs（并发20，重试3次，max_tokens=900）+ 四轮生成覆盖所有296个无use-case工具 + seo_pages use_case总数达5,930条（从3,257净增2,673条）
- SPEC-STRIPE-VERIFIED ✅ Stripe Live模式确认（2026-04-08）：.env.local STRIPE_SECRET_KEY=sk_live_ + NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=pk_live_ + 7个toolkit已有Live Price ID + 无test模式代码 + Vercel端需单独确认
